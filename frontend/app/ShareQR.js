...
export default function ShareQR({url,title="مربوعة",label="مشاركة"}){
  ...
  return <div className="share-qr"><button type="button" onClick={share}><Icon name="share" size={17}/>{label}</button>{status&&<span className="share-status" role="status">{status}</span>}{qr&&<details><summary>QR</summary><img src={qr} alt={`QR ${title}`}/><small>{url}</small></details>}</div>;
}
