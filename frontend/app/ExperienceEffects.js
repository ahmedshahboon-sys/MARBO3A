...
export default function ExperienceEffects(){
 const prefs=useRef({...defaults}),audio=useRef(null),unlocked=useRef(false);
 useEffect(()=>{
   ...
   ]).then(([a,b])=>{prefs.current={...defaults,...(a.status==="fulfilled"?a.value?.preferences:null),notification_sounds:b.status==="fulfilled"?b.value?.settings?.notification_sounds!==false:true}}).catch(()=>{});
