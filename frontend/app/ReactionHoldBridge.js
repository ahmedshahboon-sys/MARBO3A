...
export default function ReactionHoldBridge(){
  ...
  return createPortal(<div className="reaction-hold-palette" role="menu" style={{left:palette.left,top:palette.top,width:palette.width}}>{reactions.map((emoji,index)=><button key={emoji} type="button" aria-label={palette.buttons[index]?.getAttribute("aria-label")||"تفاعل"} onPointerDown={e=>e.stopPropagation()} onClick={e=>{e.preventDefault();e.stopPropagation();delete palette.picker.dataset.suppressNextLike;palette.buttons[index]?.click();close()}}>{emoji}</button>)}</div>,document.body);
}
