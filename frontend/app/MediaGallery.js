...
export default function MediaGallery({items=[],alt="وسائط",className=""}){
 ...
 const current=index===null?null:media[index],currentKind=kind(current),dataSaver=typeof document!=="undefined"&&document.documentElement.dataset.dataSaver==="true",autoplay=typeof document==="undefined"||document.documentElement.dataset.autoplayMedia!=="false";
 return <>
  <div className={`media-grid media-count-${Math.min(media.length,4)} ${className}`}>
   {media.slice(0,4).map((m,i)=>{const type=kind(m);return <button type="button" key={`${m.url}-${i}`} onClick={()=>setIndex(i)} aria-label={`فتح ${type==="video"?"الفيديو":"الصورة"} ${i+1} من ${media.length}`} className={type==="video"?"media-video-tile":""}>
    {type==="video"?<><video src={m.url} preload={dataSaver?"none":"metadata"} muted playsInline/><span className="media-video-play"><PlayIcon/></span></>:<img src={m.url} alt={`${alt} ${i+1}`} loading="lazy"/>}
  ...
  {current&&<div ref={viewerRef} className="media-viewer" role="dialog" tabIndex={-1} aria-modal="true" aria-label="عارض الوسائط" onMouseDown={e=>e.target===e.currentTarget&&setIndex(null)} onPointerDown={pointerDown} onPointerUp={pointerUp} onPointerCancel={()=>{swipe.current=null}}>
   ...
   {currentKind==="video"?<video className="media-viewer-video" src={current.url} controls playsInline autoPlay={autoplay&&!dataSaver} preload={dataSaver?"none":"metadata"}/>:<img src={current.url} alt={`${alt} ${index+1}`}/>} 
