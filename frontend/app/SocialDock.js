...
export default function SocialDock(){
 ...
 return <nav className="social-dock" aria-label="التنقل الرئيسي"><div className="social-dock-track">
  <button type="button" className="dock-more" onClick={openDrawer} aria-label="المزيد"><Icon name="more"/><span>المزيد</span></button>
  <Link prefetch href="/friends" className={active("/friends")?"active":""}><Icon name="users"/><span>الأصحاب</span></Link>
  <Link prefetch href="/home" className={`dock-create dock-home-center ${active("/home")?"active":""}`} aria-label="الرئيسية"><Icon name="home"/><span>الرئيسية</span></Link>
  <Link prefetch href="/messages" className={`dock-messages ${active("/messages")||active("/chat")?"active":""}`}><span className="dock-icon-wrap"><Icon name="message"/>{unread>0&&<b className="dock-unread-badge">{badge}</b>}</span><span>الرسائل</span></Link>
  <Link prefetch href="/rooms" className={active("/rooms")||active("/room")?"active":""}><Icon name="hash"/><span>الغرف</span></Link>
 </div></nav>
}
