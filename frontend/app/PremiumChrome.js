...
const trackedTitle=v=>{const d=v?new Date(v):null;if(!d||Number.isNaN(d.getTime()))return"إجمالي الهويات الزائرة المسجلة منذ بدء نظام التتبع";return`إجمالي الهويات الزائرة المسجلة منذ ${d.toLocaleDateString("ar-LY")}`};
export default function PremiumChrome(){
  ...
  return <header className={`v3-global-header${conversation?" v3-conversation-chrome":""}`} dir="rtl">
    ...
    <div className="v3-audience-stats" aria-label="إحصائيات مربوعة">
      <span title={trackedTitle(audience.trackingStartedAt)}><Icon name="users"/><b>{compact(audience.totalVisitors)}</b><small>زائر مسجل</small></span>
