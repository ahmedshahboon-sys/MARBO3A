...
export default function PremiumChrome(){
  const path=usePathname(),[unread,setUnread]=useState(0),[audience,setAudience]=useState({totalVisitors:0,onlineNow:0});
  ...
  return <header className={`v3-global-header${conversation?" v3-conversation-chrome":""}`} dir="rtl">
    ...
    <div className="v3-audience-stats" aria-label="إحصائيات مربوعة">
      <span title="إجمالي الزوار منذ بداية مربوعة"><Icon name="users"/><b>{compact(audience.totalVisitors)}</b><small>زائر</small></span>
      <span title="المتصلون الآن"><i className="v3-online-pulse"/><b>{compact(audience.onlineNow)}</b><small>متصل</small></span>
    </div>
