...
export function UiPageHeader({title,kicker="MARBO3A",backHref=null,children}){return <header className="ui-page-header">{backHref?<UiLinkButton href={backHref} className="ui-header-back" aria-label="رجوع"><Icon name="arrowRight"/></UiLinkButton>:<span/>}<div><small>{kicker}</small><h1>{title}</h1></div><div className="ui-header-actions">{children}</div></header>}
