...
export default function PlatformClient(){
  ...
  useEffect(()=>{
    ...
    const showNext=()=>{if(notification||!notifQueue.current.length)return;const next=notifQueue.current.shift();setNotification(next);clearTimeout(notifTimer.current);notifTimer.current=setTimeout(()=>setNotification(null),3000)};
    const onNotification=e=>{const n=e?.detail;if(!n?.id)return;notifQueue.current.push(n);if(!notification){const next=notifQueue.current.shift();setNotification(next);clearTimeout(notifTimer.current);notifTimer.current=setTimeout(()=>setNotification(null),3000)}};
  ...
  function nudgeUp(e){const p=nudgeDrag.current;nudgeDrag.current=null;if(p&&Math.abs(e.clientX-p.x)>55&&Math.abs(e.clientX-p.x)>Math.abs(e.clientY-p.y))setInstallNudge(false)}
  ...
  return <>
    {installNudge&&!installed&&<aside className="install-nudge" role="status" onPointerDown={nudgeDown} onPointerUp={nudgeUp}><img src="/brand/official/marbo3a-mark.png" alt=""/><div><b>ثبّت مربوعة كتطبيق</b><span>دخول أسرع وتجربة كاملة من الشاشة الرئيسية</span></div><button type="button" onClick={install}>تثبيت</button><button type="button" className="install-nudge-close" aria-label="إخفاء" onClick={()=>setInstallNudge(false)}><Icon name="close" size={16}/></button></aside>}
