...
export default function PlatformClient(){
  ...
  return <>
    ...
    {showIOS&&<div className="pwa-modal" onMouseDown={e=>e.currentTarget===e.target&&setShowIOS(false)}><div><button className="pwa-modal-close" onClick={()=>setShowIOS(false)} aria-label="إغلاق"><Icon name="close"/></button><img src="/logo.svg" alt="مربوعة"/><h3>ثبّت مربوعة على iPhone/iPad</h3><p>من Safari اضغط زر المشاركة، وبعدها اختار <b>إضافة إلى الشاشة الرئيسية</b> ثم «إضافة».</p><button className="pwa-done" onClick={()=>setShowIOS(false)}>تمام</button></div></div>}
  </>;
}
