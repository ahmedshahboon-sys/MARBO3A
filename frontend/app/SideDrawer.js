...
const appPaths=["/home","/feed","/search","/notifications","/friends","/messages","/rooms","/map","/settings","/u/","/profile/","/blocked","/chat/","/room/","/onboarding"];
export default function SideDrawer(){
  ...
  useEffect(()=>{const openFromChrome=()=>setOpen(true);window.addEventListener("marbo3a:open-drawer",openFromChrome);return()=>window.removeEventListener("marbo3a:open-drawer",openFromChrome)},[]);
  ...
  const items=[["/home","home","الرئيسية"],["/search","search","البحث"],["/notifications","bell","الإشعارات"],["/friends","users","الأصحاب"],["/messages","message","الرسائل"],["/rooms","hash","الغرف"],["/map","map","الخريطة"],[`/u/${me.username}`,"user","ملفي الشخصي"],["/profile/edit","image","تعديل الملف"],["/blocked","warning","المحظورون"]];
  return <><button className="drawer-trigger" onClick={()=>setOpen(true)} aria-label="فتح القائمة"><Icon name="menu" size={24}/></button>{open&&<div className="drawer-overlay" onMouseDown={e=>e.target===e.currentTarget&&setOpen(false)}><aside className="social-drawer"><header><div className="drawer-user">{me.avatar_url?<img src={me.avatar_url} alt=""/>:<span>{me.display_name?.[0]||"م"}</span>}<div><b>{me.display_name}</b><small><bdi dir="ltr">@{me.username}</bdi></small></div></div><button onClick={()=>setOpen(false)} aria-label="إغلاق"><Icon name="close"/></button></header><nav>{items.map(([href,icon,label])=><Link prefetch key={href} href={href} className={path===href||path?.startsWith(href+"/")?"active":""}><Icon name={icon}/><span>{label}</span></Link>)}</nav><footer>{isAdmin&&<><Link prefetch href="/admin"><Icon name="settings"/><span>مركز الإدارة</span></Link><Link prefetch href="/admin/readiness"><Icon name="check"/><span>فحص الجاهزية</span></Link></>}{UI_MODE==="v3"?<Link prefetch href="/settings"><Icon name="settings"/><span>الإعدادات</span></Link>:<button onClick={openSettings}><Icon name="settings"/><span>الإعدادات</span></button>}<button className="drawer-logout" onClick={logout}><Icon name="logout"/><span>تسجيل الخروج</span></button></footer></aside></div>}</>;
}
