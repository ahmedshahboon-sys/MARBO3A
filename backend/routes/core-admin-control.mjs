...
export function registerCoreAdminControl(app){
  app.get("/api/admin/control/overview",async(req,res)=>{try{
    ...
    try{
      ...
      [onlineUsers,onlineVisitors,visitorsToday]=await Promise.all([
        redis.zCard("presence:users").catch(()=>0),
        redis.zCard("presence:visitors").catch(()=>0),
        redis.pfCount(`visits:day:${day}`).catch(()=>0)
      ]);
    ...
    const [
      ...
      activeRooms,admins,moderators,activeBroadcasts,registeredUsers,activeUsers24h,rooms,
      frozen,banned,totalMessages,recentUsers,recentErrors,recentReports
    ]=await Promise.all([
      ...
      safeCount(`SELECT COUNT(*)::int c FROM users`),
      safeCount(`SELECT COUNT(*)::int c FROM users WHERE last_seen_at>=NOW()-INTERVAL '24 hours'`),
      safeCount(`SELECT COUNT(*)::int c FROM rooms`),
      safeCount(`SELECT COUNT(*)::int c FROM users WHERE account_status='frozen'`),
      safeCount(`SELECT COUNT(*)::int c FROM users WHERE account_status='banned'`),
      safeCount(`SELECT ((SELECT COUNT(*) FROM messages)+(SELECT COUNT(*) FROM direct_messages))::int c`),
    ...
    res.json({ok:true,overview:{
      ...
      live:{onlineUsers:Number(onlineUsers||0),onlineVisitors:Number(onlineVisitors||0),visitorsToday:Number(visitorsToday||0)},
      moderation:{openReports,errorsHour,errorsToday:errorsDay,frozen,banned},
      growth:{newUsersHour,newUsersToday:newUsersDay,registeredUsers,activeUsers24h},
      activity:{messagesHour,messagesToday:messagesDay,totalMessages,activeRooms24h:activeRooms,rooms},
      access:{admins,moderators},broadcasts:{active:activeBroadcasts},
