...
import {pool,redis,ensureRedis,requireAuth,tokenFrom,emitUser,ipOf,clean} from "./runtime.mjs";
...
async function notifyPassword(email,kind){if(!process.env.BREVO_API_KEY||!email)return;const reset=kind==="reset",subject=reset?"تم تغيير كلمة مرور حسابك في مربوعة":"تم تحديث كلمة مرور حسابك في مربوعة";await fetch("https://api.brevo.com/v3/smtp/email",{method:"POST",headers:{accept:"application/json","content-type":"application/json","api-key":process.env.BREVO_API_KEY},body:JSON.stringify({sender:{name:"مربوعة",email:"no-reply@marbo3a.ly"},to:[{email}],subject,htmlContent:`<div dir="rtl"><h2>مربوعة</h2><p>${reset?"تم تغيير كلمة المرور عبر الاسترجاع.":"تم تغيير كلمة مرور حسابك."}</p><p>إذا لم تكن أنت، تواصل مع الدعم فورًا.</p></div>`})}).catch(()=>{})}
