...
function bearerToken(req){return String(req.headers.authorization||"").match(/^Bearer\s+([a-f0-9]{64})$/i)?.[1]||""}
...
http.createServer=function cookieAuthCreateServer(app,...args){
  if(typeof app==="function"&&app?.use){
    // Browser clients use the HttpOnly cookie. Existing bearer-only sessions are upgraded
    // on their next API request, then the frontend replaces the old secret with a marker.
    app.use((req,res,next)=>{const c=cookieToken(req),b=bearerToken(req),a=String(req.headers.authorization||"");if(!c&&b)setCookie(res,b);if(c&&!/^Bearer\s+[a-f0-9]{64}$/i.test(a))req.headers.authorization=`Bearer ${c}`;next()});
