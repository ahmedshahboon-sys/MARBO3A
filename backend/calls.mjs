...
http.createServer=function(app,...args){
  if(typeof app==="function"&&app?.use){
    ...
    // Keep static routes above /api/calls/:id so Express never interprets "config" as a call id.
    app.get("/api/calls/config",async(req,res)=>{try{const u=await auth(req,res);if(!u)return;const iceServers=iceConfig(u.id);res.json({ok:true,iceServers,turnConfigured:Boolean(process.env.TURN_SECRET&&(process.env.TURN_HOST||process.env.TURN_EXTERNAL_IP)),credentialTtlSeconds:3600})}catch(e){console.error("call config",e);res.status(500).json({ok:false,error:"CALL_CONFIG_FAILED"})}});
