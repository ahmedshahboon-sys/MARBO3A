...
http.createServer=function(app,...args){
  if(typeof app==="function"&&app?.use){
    ...
    app.post("/api/calls",async(req,res)=>{try{
      const u=await auth(req,res);if(!u)return;
      const conv=Number(req.body?.conversationId),kind=req.body?.kind==="video"?"video":"audio";
      if(!Number.isInteger(conv)||conv<1)return res.status(400).json({ok:false,error:"BAD_CONVERSATION"});
      const c=(await pool.query(`SELECT id,user1_id,user2_id FROM direct_conversations WHERE id=$1 AND ($2=user1_id OR $2=user2_id)`,[conv,u.id])).rows[0];
      if(!c)return res.status(403).json({ok:false,error:"CHAT_FORBIDDEN"});
      const callee=Number(c.user1_id)===Number(u.id)?Number(c.user2_id):Number(c.user1_id);
      await pool.query(`UPDATE rtc_calls SET status='ended',ended_at=NOW() WHERE status IN ('ringing','answered') AND ((caller_id=$1 AND callee_id=$2) OR (caller_id=$2 AND callee_id=$1))`,[u.id,callee]);
      const id=crypto.randomUUID();
      const row=(await pool.query(`INSERT INTO rtc_calls(id,conversation_id,caller_id,callee_id,kind) VALUES($1,$2,$3,$4,$5) RETURNING *`,[id,conv,u.id,callee,kind])).rows[0];
      res.status(201).json({ok:true,call:row});
    }catch(e){console.error("create call",e);res.status(500).json({ok:false,error:"CALL_CREATE_FAILED"})}});

    app.get("/api/calls/incoming",async(req,res)=>{try{
      const u=await auth(req,res);if(!u)return;
      await pool.query(`UPDATE rtc_calls SET status='missed',ended_at=NOW() WHERE callee_id=$1 AND status='ringing' AND created_at<NOW()-INTERVAL '75 seconds'`,[u.id]);
      const row=(await pool.query(`SELECT c.*,x.username caller_username,x.display_name caller_name,x.avatar_url caller_avatar FROM rtc_calls c JOIN users x ON x.id=c.caller_id WHERE c.callee_id=$1 AND c.status='ringing' ORDER BY c.created_at DESC LIMIT 1`,[u.id])).rows[0]||null;
      res.json({ok:true,call:row});
    }catch(e){console.error("incoming call",e);res.status(500).json({ok:false,error:"CALL_LOAD_FAILED"})}});
    ...
    app.post("/api/calls/:id/signals",async(req,res)=>{try{
      const u=await auth(req,res);if(!u)return;const c=await callFor(req.params.id,u);if(!c)return res.status(404).json({ok:false,error:"CALL_NOT_FOUND"});
      const kind=String(req.body?.kind||"");if(!["offer","answer","ice"].includes(kind))return res.status(400).json({ok:false,error:"BAD_SIGNAL"});
      const recipient=Number(c.caller_id)===Number(u.id)?Number(c.callee_id):Number(c.caller_id);
      const payload=req.body?.payload;if(!payload||typeof payload!=="object")return res.status(400).json({ok:false,error:"BAD_SIGNAL"});
      const row=(await pool.query(`INSERT INTO rtc_call_signals(call_id,sender_id,recipient_id,kind,payload) VALUES($1,$2,$3,$4,$5) RETURNING id`,[c.id,u.id,recipient,kind,payload])).rows[0];
      res.status(201).json({ok:true,id:row.id});
    }catch(e){console.error("call signal",e);res.status(500).json({ok:false,error:"SIGNAL_FAILED"})}});

    app.get("/api/calls/:id/signals",async(req,res)=>{try{
      const u=await auth(req,res);if(!u)return;const c=await callFor(req.params.id,u);if(!c)return res.status(404).json({ok:false,error:"CALL_NOT_FOUND"});
      const since=Math.max(0,Number(req.query.since)||0);
      const rows=(await pool.query(`SELECT id,kind,payload,created_at FROM rtc_call_signals WHERE call_id=$1 AND recipient_id=$2 AND id>$3 ORDER BY id ASC LIMIT 200`,[c.id,u.id,since])).rows;
      res.json({ok:true,signals:rows});
    }catch(e){res.status(500).json({ok:false,error:"SIGNAL_LOAD_FAILED"})}});

    app.get("/api/calls/config",async(req,res)=>{const u=await auth(req,res);if(!u)return;const iceServers=[{urls:["stun:stun.l.google.com:19302","stun:stun1.l.google.com:19302"]}];res.json({ok:true,iceServers,turnConfigured:false})});
  }
