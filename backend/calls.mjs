...
http.createServer=function(app,...args){
  if(typeof app==="function"&&app?.use){
    ...
    app.get("/api/calls/config",async(req,res)=>{const u=await auth(req,res);if(!u)return;const iceServers=iceConfig(u.id);res.json({ok:true,iceServers,turnConfigured:Boolean(process.env.TURN_SECRET&&(process.env.TURN_HOST||process.env.TURN_EXTERNAL_IP)),credentialTtlSeconds:3600})});
  }
