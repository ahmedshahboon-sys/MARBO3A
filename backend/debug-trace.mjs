...
async function init(){
  if(!ready) ready=(async()=>{
    ...
    /* Keep session trace events separate from instrumentation.debug_events.
       The old code used the same table name with an incompatible schema. */
...
async function admin(req){
  ...
  if(!id){
    /* Durable-session fallback fixes debug access after Redis restarts. */
    ...
    if(durable){
      id=String(durable.user_id);
      await redis.set(`session:${t}`,id,{EX:7*24*60*60}).catch(()=>{});
    }
  }
  if(!id) return null;
...
function scrub(o){
  ...
  for(const[k,v]of Object.entries(o||{})){
    if(/password|token|secret|authorization|otp|code/i.test(k)) continue;
...
http.createServer=function(app,...args){
  if(typeof app==="function"&&app?.use){
    ...
    app.post("/api/debug/session/start",async(req,res)=>{
      try{
    ...
    app.post("/api/debug/session/:id/events",async(req,res)=>{
      try{
        const u=await admin(req);if(!u)return res.status(403).json({ok:false,error:"ADMIN_ONLY"});
        const sid=String(req.params.id);
        const own=(await pool.query(`SELECT 1 FROM debug_sessions WHERE id=$1 AND user_id=$2`,[sid,u.id])).rows[0];
        ...
        for(const e of events){
          const type=clean(e.type,40),path=clean(e.path,160),payload=scrub(e);
          if(/error|overlap|overflow/.test(type))errors++;
          await pool.query(`INSERT INTO debug_session_events(session_id,event_type,path,payload) VALUES($1,$2,$3,$4::jsonb)`,[sid,type,path,JSON.stringify(payload)]);
        }
    ...
    app.get("/api/debug/sessions/:id",async(req,res)=>{
      try{
        if(!await admin(req))return res.status(403).json({ok:false,error:"ADMIN_ONLY"});
        const s=(await pool.query(`SELECT * FROM debug_sessions WHERE id=$1`,[req.params.id])).rows[0];
        if(!s)return res.status(404).json({ok:false,error:"DEBUG_SESSION_NOT_FOUND"});
        ...
        const summary={
          errors:events.filter(x=>/error/.test(x.event_type)).length,
          ui:events.filter(x=>/overlap|overflow/.test(x.event_type)).length,
          api:events.filter(x=>x.event_type==="api_error").length,
