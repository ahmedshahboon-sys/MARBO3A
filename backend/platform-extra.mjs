...
http.createServer=function extraCreateServer(app,...args){
  if(typeof app==="function"&&app?.use){
    ...
    app.post("/api/reports",async(req,res)=>{const u=await auth(req,res);if(!u)return;const type=clean(req.body?.targetType,30),target=Number(req.body?.targetId),reason=clean(req.body?.reason,100),details=clean(req.body?.details,1000);if(!["user","message","room","direct_message"].includes(type)||!Number.isInteger(target)||!reason)return res.status(400).json({ok:false,error:"INVALID_REPORT"});const row=(await pool.query(`INSERT INTO reports(reporter_id,target_type,target_id,reason,details) VALUES($1,$2,$3,$4,$5) RETURNING id,created_at`,[u.id,type,target,reason,details])).rows[0];res.status(201).json({ok:true,report:row})});
    app.get("/api/admin/reports",async(req,res)=>{const a=await admin(req,res);if(!a)return;const rows=(await pool.query(`SELECT r.*,u.username reporter_username FROM reports r LEFT JOIN users u ON u.id=r.reporter_id ORDER BY CASE WHEN r.status='open' THEN 0 ELSE 1 END,r.id DESC LIMIT 200`)).rows;res.json({ok:true,reports:rows})});
