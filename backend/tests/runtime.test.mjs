...
test("admin authorization is role-based, not username-based",()=>{assert.equal(isAdmin({role:"admin",username:"Ahmed"}),true);assert.equal(isAdmin({role:"user",username:"Ahmed"}),false);assert.equal(isAdmin({role:"user",username:"user"}),false)});
