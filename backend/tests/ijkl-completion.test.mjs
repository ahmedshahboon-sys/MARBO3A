...
test("IJKL backend owns safe story and public discovery contracts",()=>{const src=read("ijkl-experience.mjs");for(const token of ["/api/stories/:id/reply","/api/stories/:id/reaction","/api/stories/mutes/:userId","/api/stories/:id/highlight","/api/public/rooms","/api/public/stats","visibility"])assert.match(src,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")))});
