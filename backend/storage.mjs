...
export async function saveObject({buffer,extension="bin",contentType="application/octet-stream"}){const rel=objectName(extension);if(driver==="local"){await fs.mkdir(path.join(root,path.dirname(rel)),{recursive:true});await fs.writeFile(path.join(root,rel),buffer,{flag:"wx"});return{key:rel,url:`${publicPrefix}/${rel}`,type:contentType,size:buffer.length,driver}}if(driver==="s3"){const key=s3Prefix?`${s3Prefix}/${rel}`:rel;await s3().send(new PutObjectCommand({Bucket:s3Bucket,Key:key,Body:buffer,ContentType:contentType,CacheControl:"public,max-age=31536000,immutable"}));return{key,url:publicS3Url(key),type:contentType,size:buffer.length,driver}}throw new Error("STORAGE_DRIVER_NOT_CONFIGURED")}
export async function deleteObject(key){const value=String(key||"").replace(/^\/+/,"");if(!value)return false;if(driver==="local"){const resolved=path.resolve(root,value);if(!resolved.startsWith(root+path.sep))throw new Error("INVALID_STORAGE_KEY");await fs.unlink(resolved).catch(e=>{if(e.code!=="ENOENT")throw e});return true}if(driver==="s3"){await s3().send(new DeleteObjectCommand({Bucket:s3Bucket,Key:value}));return true}throw new Error("STORAGE_DRIVER_NOT_CONFIGURED")}
