...
async function canSeePost(viewerId,row){if(String(viewerId)===String(row.user_id))return true;if(await isBlocked(viewerId,row.user_id))return false;const rule=row.privacy_rule||'everyone';if(rule==='everyone')return true;if(rule==='nobody')return false;if(await areFriends(viewerId,row.user_id))return true;return rule==='friends_of_friends'&&await hasMutualFriend(viewerId,row.user_id)}
async function visibleSavedRows(viewerId,rows){const out=[];for(const row of rows)if(await canSeePost(viewerId,row))out.push(row);return out}
...
http.createServer=function closureRoutesCreateServer(app,...args){
  if(typeof app==="function"&&app?.use){
    ...
    app.delete("/api/friends/:userId",async(req,res)=>{try{const user=await requireAuth(req,res);if(!user)return;const target=Number(req.params.userId);if(!Number.isSafeInteger(target)||target<=0||target===Number(user.id))return res.status(400).json({ok:false,error:"INVALID_USER"});const deleted=await pool.query(`DELETE FROM friendships WHERE status='accepted' AND ((requester_id=$1 AND addressee_id=$2) OR (requester_id=$2 AND addressee_id=$1)) RETURNING id`,[user.id,target]);if(!deleted.rowCount)return res.status(404).json({ok:false,error:"FRIENDSHIP_NOT_FOUND"});await pool.query(`INSERT INTO audit_logs(user_id,level,category,action,details) VALUES($1,'INFO','SOCIAL','FRIEND_REMOVED',$2::jsonb)`,[user.id,JSON.stringify({targetUserId:target})]).catch(()=>{});res.json({ok:true,removed:true,userId:target})}catch(error){console.error("remove friend",error);res.status(500).json({ok:false,error:"FRIEND_REMOVE_FAILED"})}});

    app.get("/api/saved/ids",async(req,res)=>{try{const user=await requireAuth(req,res);if(!user)return;const rows=(await pool.query(`SELECT s.post_id,p.user_id,COALESCE(pp.who_can_see_posts,'everyone') privacy_rule FROM post_saves s JOIN posts p ON p.id=s.post_id LEFT JOIN profile_privacy pp ON pp.user_id=p.user_id WHERE s.user_id=$1 AND p.deleted_at IS NULL ORDER BY s.created_at DESC LIMIT 2000`,[user.id])).rows,visible=await visibleSavedRows(user.id,rows);res.json({ok:true,ids:visible.map(x=>x.post_id)})}catch(error){console.error("saved ids",error);res.status(500).json({ok:false,error:"SAVED_LOAD_FAILED"})}});
    app.get("/api/saved",async(req,res)=>{try{const user=await requireAuth(req,res);if(!user)return;const rows=(await pool.query(`SELECT p.id,p.body,p.image_url,p.location_label,p.created_at,p.updated_at,u.id user_id,u.username,u.display_name,u.avatar_url,COALESCE(pp.who_can_see_posts,'everyone') privacy_rule,(SELECT COUNT(*)::int FROM post_likes l WHERE l.post_id=p.id) likes_count,(SELECT COUNT(*)::int FROM post_comments c WHERE c.post_id=p.id AND c.deleted_at IS NULL) comments_count,EXISTS(SELECT 1 FROM post_likes ml WHERE ml.post_id=p.id AND ml.user_id=$1) liked,TRUE saved,s.created_at saved_at,${mediaSql} media FROM post_saves s JOIN posts p ON p.id=s.post_id JOIN users u ON u.id=p.user_id LEFT JOIN profile_privacy pp ON pp.user_id=p.user_id WHERE s.user_id=$1 AND p.deleted_at IS NULL ORDER BY s.created_at DESC LIMIT 100`,[user.id])).rows,visible=await visibleSavedRows(user.id,rows);for(const row of visible)delete row.privacy_rule;res.json({ok:true,posts:visible})}catch(error){console.error("saved posts",error);res.status(500).json({ok:false,error:"SAVED_LOAD_FAILED"})}});
